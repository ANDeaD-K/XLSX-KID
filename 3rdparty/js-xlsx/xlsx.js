...
function s2a(s) {
	...
	// var b = a.forEach(function (item) {
	// 	item = item.charCodeAt(0) & 0xff;
	// });
	// return a;
}
...
function s2aAsync(s) {
	new Promise(function (resolve, reject) {
		if (has_buf) resolve(new Buffer(s, "binary"));
		//return s.split("").map(function(x){ return x.charCodeAt(0) & 0xff; });
		var r = s.split("").map(function(x){ return x.charCodeAt(0) & 0xff; });
		resolve(r);
	});
...
function read(blob, options) {
	//var promise = new Promise(function (resolve, reject) {
		...
		switch(options && options.type || "base64") {
			...
			case "binary": {
				var a = parse(s2a(blob), options);
				debugger;
				return a;
			}
		}
		return parse(blob, options);
		//resolve(parse(blob, options));
	//});
}
